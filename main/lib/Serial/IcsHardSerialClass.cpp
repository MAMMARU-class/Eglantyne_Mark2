...
**/
IcsHardSerialClass::IcsHardSerialClass(HardwareSerial *hardSerial, byte enpin, long baudrate, int timeout, int rxpin, int txpin)
{
...
bool IcsHardSerialClass::begin()
{
  ...
  icsHardSerial->begin(baudRate,SERIAL_8E1, rxPin, txPin);
//   icsHardSerial->setTimeout(timeOut);
  icsHardSerial->setTimeout(10);
